...
const voiceConfigSchema: z.ZodType<VoiceConfigDocument> = z.object({
  voice: z.object({
    ...
    turn: z.object({
      ...
      speechStartDebounceMs: z.number().int().nonnegative(),
      vadEngine: z.enum(['rms', 'rnnoise', 'webrtc-vad']),
      neuralFilterEnabled: z.boolean(),
      rnnoiseSpeechThreshold: z.number().min(0).max(1),
      rnnoiseEchoSpeechThresholdBoost: z.number().min(0).max(1),
      webrtcVadMode: z.union([z.literal(0), z.literal(1), z.literal(2), z.literal(3)]),
      webrtcVadSpeechRatioThreshold: z.number().min(0).max(1),
      webrtcVadEchoSpeechRatioBoost: z.number().min(0).max(1),
      assistantOutputMinRms: z.number().nonnegative(),
