...
interface VoiceSessionOverrides {
  model?: string;
  voice?: string;
  language?: string;
  openaiApiKey?: string;
...
export class VoiceSession {
  ...
  private readonly overrides: VoiceSessionOverrides;
  ...
  async connect(): Promise<void> {
    ...
    const sessionConfig: Record<string, unknown> = {
      ...
      voice: this.overrides.voice ?? this.profile.voice,
      inputAudioTranscription: {
        model: 'gpt-4o-mini-transcribe',
        language: this.overrides.language ?? 'de',
      },
      turnDetection: {
        type: 'semantic_vad',
      },
    ...
    this.session = new RealtimeSession(this.agent, {
      apiKey: this.overrides.openaiApiKey || config.openai.apiKey,
      transport: 'websocket',
      model: this.overrides.model || config.agent.model,
  ...
  private setupEventListeners(): void {
    ...
    this.session.on('agent_tool_start', (_ctx, _agent, tool, details) => {
      ...
      this.setState('thinking');
      this.emit('toolStart', tool.name, parsedArgs, callId);
    });

    this.session.on('agent_tool_end', (_ctx, _agent, tool, result, details) => {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const toolCall = (details as any)?.toolCall;
      const callId = toolCall?.callId ?? toolCall?.call_id;
      ...
      this.emit('toolEnd', tool.name, result, callId);
    });
