"""
Benchmark parakeet-mlx (standalone) for German speech recognition.

Uses the same German test WAV files from packages/voice-runtime/data/test/
for direct comparison with the mlx-audio STT benchmark.

parakeet-mlx is the dedicated Parakeet implementation by senstella,
separate from mlx-audio's built-in Parakeet adapter.
...
Usage:
    ...
    uv run python bench_stt_parakeet.py
    uv run python bench_stt_parakeet.py --model mlx-community/parakeet-tdt-0.6b-v3
"""
...
def run_benchmark(model_id: str):
    ...
    print(f"\n{BOLD}{'=' * 70}{RESET}")
    print(f"{BOLD}  STT Benchmark: parakeet-mlx (standalone){RESET}")
    print(f"{BOLD}{'=' * 70}{RESET}")
    print(f"  {CYAN}Model:{RESET}      {model_id}")
    print(f"  {CYAN}Test files:{RESET} {len(turns)} German turns from voice-runtime")
    print(f"  {CYAN}Backend:{RESET}    parakeet-mlx (senstella, Apple MLX)")
    print(f"{BOLD}{'─' * 70}{RESET}\n")
    ...
    _ = model.transcribe(warmup_path)
    ...
    print(f"  Backend:               parakeet-mlx (standalone)")
...
def main():
    parser = argparse.ArgumentParser(description="Benchmark parakeet-mlx STT for German")
    parser.add_argument(
        "--model",
